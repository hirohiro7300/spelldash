...
- 訳に対して別の正しい英単語を打っても不正解にしない。「話す」に talk と打つと1文字目で
  不正解になり、答えが表示され、思い出せなかった記録まで付いていた（受験生の指摘）
- js/answers.js: 全カードの訳から「同じ訳を持つ語」の表を作る（延べ2,080語）。
  カードの `accept` に書いた別解も候補に加える
- 打鍵の判定を出題語1本ではなく候補すべてに対して行う。短い別解で長い出題語を
  打ち切らない（advertisement を ad で止めない）
- 別解で打ち終わったら減点も×記録もせず「talk も「話す」。この問題の語は speak」と案内し、
  出題語を打ってもらう。答えを見た後は出題語だけを受け付ける
- つづり違い（favourite / favorite、maths / math）は同じ語として、打ち直させず正解にする
- 初級7パックに人手で別解を 221語追加（大きい→large、ごみ→garbage/trash/rubbish、
  米英差の elevator/lift、gasoline/petrol など）
...
- 道のコースに「TOEIC 英単語 1,250語（TSL）」「ビジネス英単語 1,750語（BSL）」を追加（コースは9本）
- 教材ライブラリの分野グループを畳む形に（押すと開く。検索中と追加済みを含むグループは開く）
- 訳の中に見出し語が入っていた20語を伏せ字に直した（「（per capita）一人当たり」→「一人当たり（per ___）」）。答えが出題文に見えると思い出す練習にならないため。検証で弾くようにし、docs/PACK_FORMAT.md にも明記
- 訳の紛らわしさの判定を js/jaAmbiguity.js に切り出し、かっこ書きや活用語尾を落とすと同じになる訳も拾うようにした
- 単語詳細が、開いたパックの版の訳・例文を出すようにした（同じ語が複数パックにあるとき）
...
- 虹色文字を2トーン（打った文字＝朱、未入力＝薄墨）に。UI の絵文字を撤去し js/icons.js の線画に
- ホーム: ミッション・週間レポート・ヒーロー・レベルバーを外し、道とスタートを主役に。連続日数と今週だけ
- 廃止: シェア画像（今日のセット／Daily／累計）、BGM、よく間違えるキー、語根ファミリー、正解時にも発音の設定、インストール案内の文面。覚え方を作るはログイン時のみ表示
- 音声: 端末の最も自然な英語音声を自動選択（Natural > Premium > Enhanced > Google > Siri）、声の選択と試聴、速さ3段階、発話前の正規化、初回操作でのアンロック。効果音を短いクリック音に
- 出題中のカテゴリの版を優先して語を引く（findWordIn）。同 id の語が基本カテゴリとレベル別パックにある時、品詞・例文がパック版になる
- NGSL 1.2（CC BY-SA 4.0）を 24 パックに分割する生成スクリプトと、ngsl01〜24（2,809位まで・2,857語）を追加。パックページに出典、道のコース「基本英単語 2,800語（NGSL）」（24セクション）
...
- **バグ修正**: easy 0語のカテゴリ（IT）をLv1〜4で選ぶとChallengeの出題が空になりクラッシュする問題
  - カテゴリ内に解放難易度の単語が無い場合、そのカテゴリで最も易しい難易度を自動解放（filterByAllowedLevels）
  - Study補充にも同じ救済を適用（カテゴリ選択の意思を尊重し「何も出ない」を作らない）
- **CI導入**: .github/workflows/ci.yml — PRとmainへのpushごとに単語データ検証＋E2E22件が自動実行
- 常設E2Eを22件に拡張（IT×Lv1の枯渇ケースを追加）

## 2026-07-15 (リザルトパネル＋ヘッダーストリーク＋ヒーロー統一) ※承認待ち・デプロイ凍結中

- Challenge/Daily終了時にリザルトパネルを表示（スコア/思い出せず/ミスタイプ/速度/獲得XP）
  - 🏆ベストスコア更新バッジ、[もう一回]（Challenge）/[結果をシェア]（Daily）/[苦手をStudyで復習]の次アクション導線
- 全ページのヘッダーに🔥ストリークを常時表示（今日未クリアはグレー＝どのページでも損失回避が目に入る）
- battle/stats/profileのヒーローもコンパクト化（ホームと統一）
- 常設E2Eを20件に拡張（リザルトパネル・ヘッダーストリークを追加）
- ※Vercelデプロイ制限（今日28回のデプロイで上限）のため、解除後にPR#28と合わせて1回で反映予定

## 2026-07-15 (モード選択画面＋単語一覧の折りたたみ) ※承認待ち

- ホーム最上部を「モード選択」に: 📚Study / ⏱Challenge / ⚡Daily Dash / ⚔️Battle の4枚タイル（2x2）
  - タイルを選んだ瞬間にそのモードが開始（選択画面としてふるまう）
  - Dailyタイルは完了後「✓ 今日は完了（スコア N）」表示になり、クリックで結果カードへスクロール
  - Battleタイルは battle.html へのリンク
- 学習データページの単語一覧を折りたたみに（既定で閉、開くと検索・カテゴリ絞り込み）

## 2026-07-15 (創業者フィードバック対応3点)

ローンチ直後の創業者の実プレイフィードバック（①何のサイトか分からない ②残り時間が見づらい ③単語が難しい）への対応。

- **ホームをゲーム主役の2カラムに再構築**
  - PC: ゲームカードがファーストビューに入り、周辺情報（ストリーク/Daily/ミッション/レベル/カテゴリ/比率）は右サイドバーへ
  - モバイル: ゲームが最上部、ウィジェットは下へ（1カラム）
  - ヒーローをコンパクト化（バッジ削除・見出し縮小）
- **大型残り時間タイマー**: Challenge/Dailyプレイ中、ゲームカード左上に大きく表示。残り10秒で赤＋パルス
- **単語難易度のレベル解放**: Lv1-4=easyのみ / Lv5-9=+normal / Lv10+=hard解放
  - 適用は新出単語の導入のみ（Study補充・Challenge抽選・ミッションNew）。既習語は常に出題、**Daily Dashは全員共通のため適用外**（公平性維持）
  - レベルアップ時に「🔓 新しい難易度の単語が解放！」と表示（Lv5/Lv10）
- 検証: 常設E2E 16/16＋新規検証9項目（ファーストビュー内にゲーム/サイドバー配置/タイマー表示切替/Lv1はeasyのみ/Lv10+でhard出現/モバイル1カラム・横スクロールなし）
...
- supabase.jsを動的import化。CDN（esm.sh）障害時でもアプリが起動し、ゲームはLocal Firstで動作
  ...
  - SDKロード後はauth/from()呼び出しを実クライアントへ透過委譲（ビルダー呼び出しは記録→再生方式）
  - ロード失敗時は未ログイン相当のスタブ応答（同期系は全て失敗許容の設計済み）
- analytics.sqlに「毎日これ1本」のKPIサマリー（直近7日を1画面）とチートスコア検知クエリを追加
- 検証: CDN到達不能環境で起動・プレイ・ログイン操作の無事故、モックSDKで委譲・再生・UI描画の正確性をE2E確認

## 2026-07-15 (初回体験改善: 易しい単語から開始)

- 学習履歴が全く無い初回セッションのみ、最初の3語を「短い easy 単語」（egg/tea/sun等）に固定
- 狙うKPI: D1継続率・初回離脱。「最初の1語が分からない」体験を構造的に排除し、30秒以内の成功体験を保証
- 2回目以降・既存ユーザーには一切影響しない（統計が1語でもあれば発動しない）
...
- docs/LAUNCH.md: SNSローンチプラン（X告知文3案・ターゲット別訴求・投稿運用・計測方法）
- 性能監査: 初期ロード合計 約150KB（HTML9.5KB+CSS37KB+JS+単語JSON遅延111KB）で軽量。
  OG画像353KBはSNSクローラーのみが取得しページ表示に影響なし → 軽量化不要と判断

## 2026-07-15 (継続フック3点)

- レベルバーに次の称号予告「あと Nレベルで「◯◯」」を表示（モバイルでは省略）
- ミッション完了表示に「明日も新しいミッションが届きます 🌅」を追加（完了を明日への予告に変える）
- プロフィールの学習サマリーにストリークシールド保有数を追加
