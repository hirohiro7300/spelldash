...
## 2026-09-17 第7回（例文 3,022文）

英単語カード共通の例文（`ex`／`exJa`／`exForm`、仕様は docs/PACK_FORMAT.md）。執筆はカテゴリ／パックごとに別エージェント（共通ブリーフ: 1文70字以内・見出し語かその語形を含む・そのカテゴリの学習者より難しい語を使わない・訳は自然な日本語40字以内・固有名詞は Kyoto/Tokyo/Osaka/Ken/Emi/Mike/Ms. Sato・同じ文を2語で使わない・数字は英単語）。レビューは執筆者と別のエージェントが6観点（自然さと文法／語義が ja と一致／レベル／訳の自然さ／exForm の要否／重複・固有名詞・長さ）で全数通読し、その場で修正。

### 基本8カテゴリ（1,101文）

| ファイル | 修正 | 主な修正内容 | 要確認 |
|---|---|---|---|
| ads / business / toeic | 2 / 8 / 6 | CVR を「ページ」と比べていた文、訳の重複語（予測／見通し）、敬体の統一、直訳調の訳 | procurement（部署の意味で使用） |
| it / daily / travel | 4 / 1 / 0 | 「端末を開いて」→「ターミナルを開いて」、USB の差し込み口→ポート、protocol の単複、夜／夕食の重複 | terminal（ja が 端末） |
| junior / highschool | 0 / 2 | promise + O + to不定詞 → that 節、指示対象の無い it | danger（a danger of の有無） |

### 教科書英語 4本（480文）

| パック | 修正 | 主な修正内容 | 要確認 |
|---|---|---|---|
| elem-english | 2 | 他パックの別の語と同じ文（My mother is a nurse.／This cake is delicious.）を解消 | happy（訳が うれしい） |
| jhs-english1 | 9 | 一回きりの未来を現在形で言う不自然な文、他の語と同じ文 8件 | — |
| jhs-english2 | 2 | too…to（中3）を避ける、他の語と同じ文 | important／safe の It is … to は中2として許容 |
| jhs-english3 | 0 | — | victim（訳が 被災者） |

### 英検 7本・TOEIC 5本（1,441文）

| パック | 修正 | 主な修正内容 | 要確認 |
|---|---|---|---|
| eiken5 / 4 / 3 / 2 | 2 / 0 / 1 / 0 | yesterday を名詞主語で使っていた文、I see three pigs の不自然さ、直訳の訳 | frustration（ja 欲求不満 と文の 不満） |
| eikenp2 / p1 / 1 | 1 / 0 / 8 | 燃費→燃料、訳の指示対象（学説→研究）、副詞と誤読される訳、数字を漢数字に統一 | haughty（訳に部下が2回） |
| toeic500 / 600 / 730 / 860 / 990 | 2 / 4 / 8 / 4 / 6 | three new staff → staff members、ensure を 確認 と訳していた、prevail の語義（優先する→普及する）、厳秘→極秘、訳の主語の取り違え | 数字表記（漢数字／算用数字）がファイル間で不統一 |

### まとめ（第7回）

- 24ファイル・3,022文を全数レビュー。修正 72文。英文の文法誤りはほぼ無く、修正の中心は (1) 他の語と同じ文の重複（教科書英語と英検5級の間で多発）、(2) 訳が直訳・見出し語の意味が見えない、(3) 語義が ja と別の意味（prevail・ensure）。
- 次回の執筆ブリーフに追記する: (1) 同じレベル帯の他パックの文と重複しない（書き終わったら全ファイル横断で重複を検査）、(2) 訳に見出し語の意味を必ず残す（自然さのために別の語に置き換えない）、(3) 数字は英文では単語、訳では漢数字に統一。
- 執筆時の落とし穴: `data/packs/*.json` は1項目1行の整形だったため差し込みスクリプトが黙って何もしなかった → スクリプト側で `words` だけを1行1語に直してから差し込むように修正（内容不変を検証、何も差し込めなければ失敗）。
