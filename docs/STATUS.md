# SpellDash 現状整理（2026-07-15 自走セッション完了時点）

## 直近の追加（2026-07-15 自走セッション: 5機能を本番反映）

1. **Daily Dash**（PR#3）: 1日1回・全ユーザー共通問題の60秒チャレンジ。日付シードで決定論的に生成
2. **結果シェア**（PR#4）: Wordle式テキスト（絵文字グリッド🟩🟨⬛＋スコア＋ストリーク＋URL）をWeb Share API/クリップボードで共有
3. **PWA化**（PR#5）: manifest＋SW（Network First・オフライン対応）＋SDアイコン4種。ホーム画面追加可能に
4. **単語品質チェック**（PR#6）: 全1000語検証。機械検証0件、訳語修正2件（refactor/enterprise）＋表記ゆれ統一2件
5. **学習の伸び可視化**（PR#7）: スコア推移SVGグラフ＋思い出し成功率カード＋セッションログ記録開始

前日(07-14) には Streak Guard（ストリーク常時表示＋シールド、PR#1）とモバイル入力対応（PR#2）を反映済み。

**Wordle型バイラルループの現状**: 毎日同じ問題✓ → 結果シェア✓ → 残るは「ランキング」（play_sessionsのdailyレコードが土台。公開範囲設計が必要）
