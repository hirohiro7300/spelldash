# SpellDash 現状整理（2026-07-15 Daily Dash完了時点）

## 直近の追加（Daily Dash: 日替わりチャレンジ）

- 1日1回・全ユーザー共通問題の60秒チャレンジ。日付シードで決定論的に生成（js/dailyChallenge.js）
- ホームに専用カード（未挑戦=挑戦ボタン / 完走後=スコア＋「また明日」）
- 完走ボーナス+30XP、play_sessionsへ mode:"daily" で記録
- **Wordle型バイラルループ（毎日同じ問題→結果シェア→友達参入）の1ピース目。次はシェア機能とランキング**
